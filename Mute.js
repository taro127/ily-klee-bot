...
if (CommandType === 'prefix' || CommandType === 'both') {
  ...
  // Checks to see if the message starts with (prefixmute)
  let message = `${context.params.event.content}`;
  if (message.startsWith(`${process.env.COMMANDPREFIX}mute`)) {
    ...
    if (
      ...
      if (regEx.test(message)) {
        ...
        // Grabs the user and reason for the mute
        let userId = context.params.event.mentions[0].id;
        let reason = message.split(' ').slice(2).join(' ');


        // Sends a DM to notify the user that they were Muted
        await lib.discord.users['@0.1.1'].dms.create({
          ...
          embed: {
            type: 'rich',
            title: 'Muted',
            description: `Hey! You've been muted in the ${process.env.SERVERNAME}.`,
            color: 0xd35269,
            fields: [
              {
                name: 'Reason',
        ...
        // Logs the mute to the Log Channel
        await lib.discord.channels['@0.1.1'].messages.create({
          ...
          embed: {
            type: 'rich',
            title: 'Mute',
            description: `A User was Muted on the ${process.env.SERVERNAME}!`,
            color: 0xd35269,
            fields: [
              {
                name: 'User | 👤',
        ...
        // Mutes the specified user in the guild
        ...
        // Notify's the mute where it took place
        await lib.discord.channels['@0.1.0'].messages.create({
          ...
          embed: {
            ...
            footer: {
              text: `Muted by ${context.params.event.author.username}`,
            },
          },
        });
      } else {
        ...
        await lib.discord.channels['@0.2.0'].messages.create({
          channel_id: `${context.params.event.channel_id}`,
          content: `> That's invalid formating. Please use: **${process.env.COMMANDPREFIX}mute <user> <reason>**`,
          message_reference: {
      ...
      await lib.discord.channels['@0.2.0'].messages.create({
        channel_id: `${context.params.event.channel_id}`,
        content: `> <@!${context.params.event.author.id}>** | You Don't have permission to use \`${process.env.COMMANDPREFIX}mute\`**`,
        message_reference: {
          message_id: `${context.params.event.id}`,
          channel_id: `${context.params.event.channel_id}`,
          guild_id: `${context.params.event.guild_id}`,
